...
const Dropdown = ({ items }: { items: NavigationItemSliceDefaultItem[] }) => {
  return (
    <div className="invisible absolute right-0 z-10 flex flex-col gap-4 rounded-sm bg-primary py-4 px-8 opacity-0 drop-shadow-lg transition-opacity group-hover:visible group-hover:opacity-100">
      {items.map((item) => {
        return (
          <Link
